...
func _physics_process(delta):
	match state:
		...
		STATE.JUMP:
			...
			applyGravity(delta)
			if $Sprite / JumpFloorCast.is_colliding():
				velo.x = $Sprite.scale.x * maxSpeed * 3
			else :
				velo.x = 0
			velo = move_and_slide(velo, Vector2.UP)
			if is_on_floor() and $JustJumped.time_left == 0:
				state = STATE.CHARGE
		STATE.DETECT:
			$AnimationPlayer.play("Idle")
			applyGravity(delta)
...
func checkForPlayer():
	if $Sprite / PlayerCast.is_colliding():
		...
		$DetectTimer.start()
		SFX.play("SpiderAlert", rand_range(0.9, 1.1))
		
func checkForJump():
	if $Sprite / JumpCast.is_colliding() and $JumpCoolDown.time_left == 0:
		state = STATE.JUMP
		velo.y = jumpHeight
		$JustJumped.start()
		$JumpCoolDown.start()
...
func _on_StateTimer_timeout():
	if state == STATE.IDLE:
		...
		if (global_position.distance_to(player.global_position) > deagroDist
		 and not $Sprite / PlayerCast.is_colliding()):
			state = STATE.IDLE
...
func createJumpDust():
	for i in 10:
		var dust = Utils.instanceScene(load("res://Effects/DustEffect.tscn"), global_position + Vector2(i - 5, rand_range( - 1, 1)))
		dust.velo = Vector2(rand_range(0, 80 * $Sprite.scale.x), rand_range(0, - 80))
...
func _on_EnemyStats_enemyDied():
	...
	SFX.play("SpiderDeath", rand_range(0.9, 1.1))
